...
public class Logistics {
    ...
    public static void solve(int graph_size, int start, int n_dests, int[] dest, int n_edges, int[] from, int[] to, int[] cost){
        ...
        for(int i = 0; i < graph_size; i++){
            store.impose(new SumInt(store, getColumns(paths).get(i), "<=", new IntVar(store, 1, 1)));
        }

        // Du får inte lämna en nod du aldrig kommit till.
        for(int i = 0; i < graph_size; i ++){
            if(i!=start-1) {
                PrimitiveConstraint c1 = new SumInt(store, getColumns(paths).get(i), ">", new IntVar(store, 0, 0));
        ...
        for(int i = 0; i < n_dests; i++){
            store.impose(new SumInt(store, getColumns(paths).get(dest[i]-1), ">=", new IntVar(store, 1, 1)));
        }
